...
// Watch for canvas instance to be ready
watch(() => canvasStore.stageInstance, (newStage) => {
  if (newStage) {
    // These events are now managed in the canvas-konva.js store to keep components clean
  }
}, { immediate: true })
...
onMounted(() => {
  ...
  // Load a project from local storage if available
  // This is now handled within the store's initialization logic
  // canvasStore.loadFromLocalStorage('audio-graphic-design-current-project')
})
</script>
