    ...
    <ion-content>
      <div class="flex flex-col lg:grid lg:grid-cols-3 bg-secondary min-h-screen w-full px-6 sm:px-[80px] py-8">
        ...
        <div class="order-1 lg:order-2 col-span-2 lg:px-7 h-full w-full text-center sm:text-left">
          <!-- Profile header -->
          <div
            class="sm:bg-primary sm:rounded-md sm:shadow-2xl mb-6 sm:grid grid-cols-3 items-center gap-4 p-6"
          >
            <div
              class="grid-cols-2 sm:col-span-2 sm:grid items-center px-5 gap-4 col-span-2 flex flex-col justify-center space-y-3"
            >
              <!-- Profile image -->
          ...
          <div class="bg-white rounded-md p-4">
            <ion-list>
              ...
              <ion-item>
                ...
                <ion-text slot="end">{{ userStore.gender || 'Not set' }}</ion-text>
              </ion-item>

              <ion-item>
                <ion-label>Language</ion-label>
                <ion-text slot="end">{{ userStore.language || 'Not set' }}</ion-text>
              </ion-item>

              <ion-item>
                <ion-label>Date of Birth</ion-label>
                <ion-text slot="end">
                  {{ userStore.formattedDob || 'Not set' }}
                </ion-text>
              </ion-item>
            </ion-list>
          </div>
