<template>
  ...
  <div class="fixed inset-0 bg-slate-100 dark:bg-slate-900 flex flex-col touch-pan-x touch-pan-y touch-pinch-zoom">
    ...
    <div v-else>
    <!-- Header Bar -->
    <div class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-3 md:px-8 lg:px-10 xl:px-14 py-2 md:py-4" :style="windowWidth < 768 ? 'position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; z-index: 50 !important; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1) !important; min-height: 100px !important; padding-top: 70px !important;' : 'position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; z-index: 50 !important; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1) !important; min-height: 80px !important;'">
      
      <!-- Mobile Layout (stacked, compact) -->
      <div v-if="windowWidth < 768" class="flex flex-col gap-1.5">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            title="Go Back"
            @click="$router.push({ name: 'Receipt', query: $route.query })"
          >
          ...
          <div class="flex-1 py-2 min-w-0">
            <h1 class="text-base font-bold text-slate-900 dark:text-white truncate">Receipt Preview</h1>
          </div>
        </div>
        ...
        <div class="flex items-center gap-1">
          ...
          <!-- Export Button -->
          <button
            class="px-2 py-1 flex-1 bg-purple-600 hover:bg-purple-700 text-white rounded text-[10px] font-medium transition-colors"
        ...
        <!-- Row 3: Copies and Undo/Redo Controls -->
        <div class="flex items-center gap-2">
          <!-- Copies Control -->
          <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-700 rounded p-1">
            <input
              ...
              max="100"
              step="1"
              class="w-10 h-6 text-[11px] text-center bg-white dark:bg-slate-600 border border-slate-300 dark:border-slate-500 rounded text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-cyan-500"
      ...
      <div v-if="windowWidth >= 768" class="flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3 min-w-0 flex-1">
          <!-- Back Button -->
          <button
            class="hidden md:flex p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded transition-colors items-center justify-center"
            title="Go Back"
            @click="$router.go(-1)"
          >
            <svg class="w-4 h-4 text-slate-700 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="min-w-0 flex-1">
            <h1 class="text-lg font-bold text-slate-900 dark:text-white">Receipt Preview</h1>
          </div>
        </div>
          ...
          <div class="flex items-center gap-3">
            <!-- Copies Control -->
            <div class="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 rounded p-1.5">
              <span class="text-xs font-medium text-slate-700 dark:text-slate-300 px-1">Copies:</span>
              <input
                v-model.number="totalCopies"
                type="number"
                min="1"
                max="100"
                step="1"
                class="w-14 h-7 text-xs text-center bg-white dark:bg-slate-600 border border-slate-300 dark:border-slate-500 rounded text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-cyan-500"
                title="Number of copies to generate"
                @input="validateCopiesInput"
                @blur="validateCopiesInput"
              />
            ...
            <!-- Undo/Redo Controls -->
            <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-700 rounded p-1.5">
              <button
                :disabled="!canUndo"
                class="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-600 rounded transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                title="Undo (Ctrl+Z)"
                @click.stop="undo"
              >
                <svg class="w-4 h-4 text-slate-700 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="showMobileExportPopup || showDesktopExportPopup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click="closeMobileExportPopup(); closeDesktopExportPopup()">
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden" @click.stop>
        ...
        <div class="p-4 space-y-4">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <button
                class="flex items-center gap-3 p-2.5 bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors border border-slate-200 dark:border-slate-600 group text-left"
                :disabled="isExporting"
                @click="handleExportJPEG(totalCopies > 1 ? 'all' : 'current'); closeMobileExportPopup(); closeDesktopExportPopup()"
              >
                <span class="text-xl group-hover:scale-110 transition-transform">💾</span>
                <div>
          ...
          <!-- Share -->
          <div>
            <div class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Share</div>
            <button
              class="w-full flex items-center gap-3 p-2.5 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition-colors border border-blue-200 dark:border-blue-700 group text-left"
              @click="shareToSocialMedia('share'); closeMobileExportPopup(); closeDesktopExportPopup()"
              title="Share receipt"
            >
              <span class="text-xl group-hover:scale-110 transition-transform">📤</span>
              <div>
                <div class="text-sm font-medium text-blue-700 dark:text-blue-300">Share Receipt</div>
                <div class="text-[10px] text-blue-600 dark:text-blue-400">Share with others</div>
              </div>
            </button>
          </div>
        </div>
    ...
    <button
      ...
      @click.stop="showZoomControl = true"
      class="fixed right-4 top-[200px] md:top-28 z-40 p-2 bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
    ...
    <!-- Floating Zoom Control - Volume Style -->
    <div
      v-if="showZoomControl"
      ref="zoomControlRef"
      class="fixed right-4 top-[200px] md:top-28 z-40 flex flex-col items-center gap-2 bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 px-2 py-3 select-none"
      @click.stop
    >
      <!-- Plus -->
      <button
        :disabled="zoomLevel >= maxZoom"
        class="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        title="Zoom In (+)"
        @click="zoomIn"
      >
        <svg class="w-4 h-4 text-slate-700 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div
        ref="sliderTrackRef"
        class="relative w-1.5 h-24 bg-slate-200 dark:bg-slate-700 rounded-full cursor-pointer touch-none"
        @touchstart.stop="handleSliderTouchStart"
        @touchmove.stop.prevent="handleSliderTouchMove"
        @touchend.stop="handleSliderTouchEnd"
        @mousedown.stop="handleSliderMouseDown"
        @mousemove.stop="handleSliderMouseMove"
      >
        <!-- Fill with bounce animation -->
        <div
          class="absolute bottom-0 w-full bg-blue-500 rounded-full transition-all duration-150 pointer-events-none"
          :class="{ 'scale-y-110': bounce }"
          :style="{ height: zoomPercent + '%' }"
        ></div>

        <!-- Thumb -->
        <div
          class="absolute left-1/2 -translate-x-1/2 w-4 h-4 bg-white dark:bg-slate-300 rounded-full shadow cursor-pointer transition-all pointer-events-none"
          :style="{ bottom: `calc(${zoomPercent}% - 8px)` }"
        ></div>
      </div>

      <!-- Percentage Display -->
      <span class="text-[10px] font-bold text-slate-700 dark:text-slate-300 min-w-[32px] text-center">
      ...
      <!-- Fit to Screen -->
      <button
        class="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-all active:scale-95 mt-0.5 border-t border-slate-200 dark:border-slate-600"
        title="Fit to Screen"
        @click="autoFitZoom"
      >
        <svg class="w-3.5 h-3.5 text-slate-700 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section 
      ...
      class="fixed top-[180px] md:top-[140px] left-0 right-0 overflow-auto py-4" 
      :style="{ 
        bottom: '60px', 
        zIndex: '10',
        display: 'flex',
    ...
    >
      ...
      >
        ...
        <ReceiptTemplate1
          ...
          :invoice-width="invoiceWidth"
          :invoice-height="invoiceHeight"
          :enable-text-background="enableTextBackground"
          ...
          :organization-sub-name="organizationSubName"
          :organization-receipt-sub-name="organizationReceiptSubName"
          :head-office-address="headOfficeAddress"
          :head-office-phone="headOfficePhone"
          :show-page-numbers="showPageNumbers"
          :current-invoice-number="currentInvoiceNumber"
          :auto-date="autoDate"
          :naira="naira"
          :formatted-naira="formattedNaira"
          :color-styles="colorStyles"
          :signature-image1="signatureImage1"
          :signature-image2="signatureImage2"
          v-model:received-from="receivedFrom"
          v-model:sum-of="sumOf"
          v-model:sum-of2="sumOf2"
          v-model:payment-for="paymentFor"
          v-model:payment-for2="paymentFor2"
          @sum-of-overflow="handleSumOfOverflow"
          @sum-of2-input="handleSumOf2Input"
          @payment-for-overflow="handlePaymentForOverflow"
          @payment-for2-input="handlePaymentFor2Input"
        ...
        <ReceiptTemplate3
          ...
          @logo-error="handleLogoError"
        />
        <!-- End of Receipt Template Components -->
</div>
<!-- End of wrapper -->
    </section>
  ...
  <!-- End of bg-slate and outer fixed div -->
        ...
        <div class="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
              ...
              <div class="mt-4">
                <label for="receipt-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Receipt Name
                </label>
                <input
                  id="receipt-name"
...
import ReceiptTemplate1 from '../../components/templates/ReceiptTemplate1.vue';
import ReceiptTemplate2 from '../../components/templates/ReceiptTemplate2.vue';
import ReceiptTemplate3 from '../../components/templates/ReceiptTemplate3.vue';
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedTemplate = ref('template3');
    
    // Computed ref that resolves to the child template's invoiceRef DOM element
    const invoiceRef = computed(() => templateComponentRef.value?.invoiceRef || null);

    const handleLogoError = (e) => {
      console.warn('Logo failed to load in receipt template:', e);
    };
    ...
    const handleTouchMove = (e) => {
      if (e.touches.length === 1) {
        ...
        zoomLevel.value = Math.min(
          maxZoom,
          Math.max(minZoom, startZoom + deltaY * sensitivity)
        );
      } else if (e.touches.length === 2) {
        e.preventDefault();
    ...
    onMounted(() => {
      ...
      // Add click-outside listener for zoom control
      document.addEventListener('click', (event) => {
        if (showZoomControl.value && zoomControlRef.value && !zoomControlRef.value.contains(event.target)) {
    ...
    const cleanupCorruptedLocalStorage = () => {
      try {
        const keysToCheck = ['invoicePreviewData', 'invoiceQuickSettings', 'sdp_authenticated_member'];
        let cleanedCount = 0;
        ...
        try {
          ...
          const essentialKeys = ['sdp_authenticated_member'];
          const backup = {};
    ...
    // Receipt-specific reactive variables - NOW PER-PAGE
    ...
    // Formatted naira with comma separators and 2 decimal places
    const formattedNaira = computed(() => {
      if (!naira.value || naira.value === 0) return '';
      return Number(naira.value).toLocaleString('en-US', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      });
    });
    
    // Size settings - Default to 6x6 inches
    const invoiceWidth = ref(6);
    const invoiceHeight = ref(6); // Default to 6 inches for 6x6 invoice
    ...
    watch(invoiceWidth, (newWidth) => {
      if (newWidth < 5) {
        invoiceWidth.value = 5;
      }
    });
    
    watch(invoiceHeight, (newHeight) => {
      if (newHeight < 5) {
        invoiceHeight.value = 5;
      }
    });
    
    // Text background watermark
    const enableTextBackground = ref(true); // Always enabled by default
    const watermarkType = ref('both'); // Always show both logo and organization name
    ...
    const watermarkOpacity = ref(0.5); // Fixed at 50% opacity
    const watermarkFontSize = ref(6); // Default font size in px
    const watermarkVisible = ref(true); // Always visible
    ...
    // Calculate zoom percentage for slider display
    ...
    // Zoom controls with advanced features
    const zoomOrigin = ref({ x: 50, y: 50 });
    
    const zoomStep = 0.1;
    const snapLevels = [0.5, 1, 1.5];
    
    // Snap zoom magnetization
    const snapZoom = () => {
      const threshold = 0.03; // Reduced to allow easier escape from snap points
      
      for (const snap of snapLevels) {
        if (Math.abs(zoomLevel.value - snap) < threshold) {
          zoomLevel.value = snap;
          break;
        }
      }
    };
    ...
    // Helper function to download receipt image for sharing
    ...
    const shareToSocialMedia = async (platform) => {
      ...
      try {
        ...
        // Store original styles from both receipt and parent
        ...
        if (orgElement) {
          orgElement.style.fontFamily = '"Arial Black", "Arial Bold", Arial, "Helvetica Neue", Helvetica, sans-serif';
          orgElement.style.fontWeight = '900';
          orgElement.style.fontStretch = 'normal';
          orgElement.style.textTransform = 'uppercase';
          orgElement.style.letterSpacing = '0.05em';
        }
        ...
        // Generate HIGH-QUALITY receipt image using same settings as JPEG export
        ...
        if (isNative) {
          ...
          try {
            ...
            alert('❌ Failed to share receipt. Please try again or check app permissions.');
          }
        } else {
          ...
          alert('📸 Receipt image downloaded! You can share it manually.');
        }
    ...
    // Define event handlers in component scope for proper cleanup
    const handleWindowFocus = () => {
      console.log('🔄 Window focused, reloading signatures...');
      loadSignatures();
    };

    const handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log('🔄 Tab visible again, reloading signatures...');
        setTimeout(() => {
          loadSignatures();
        }, 200);
      }
    ...
    const isRestoring = ref(false); // Flag to prevent watchers during restoration
    
    const canUndo = computed(() => {
      const result = historyIndex.value > 0;
      console.log('🔍 canUndo:', result, 'historyIndex:', historyIndex.value, 'historyLength:', history.value.length);
      return result;
    });
    const canRedo = computed(() => {
      const result = historyIndex.value < history.value.length - 1;
      console.log('🔍 canRedo:', result, 'historyIndex:', historyIndex.value, 'historyLength:', history.value.length);
      return result;
    });
    
    // Force initial state save after all components are ready
    setTimeout(() => {
      saveState();
      console.log('✅ Initial undo/redo state saved. History:', history.value.length, 'CanUndo:', canUndo.value);
    }, 1000);
    ...
    const saveState = () => {
      console.log('💾 saveState called, isRestoring:', isRestoring.value);
      // Don't save state during restoration
      ...
      const currentState = {
        ...
        // Receipt-specific fields (per-page data)
        date: date.value,
        ...
        sumOf: sumOf.value,
        sumOf2: sumOf2.value,
        // Store entire current page data for complete restoration
        currentPageData: JSON.parse(JSON.stringify(pageData.value[currentPage.value] || {})),
    ...
    // Undo function
    const undo = () => {
      console.log('⬅️ Undo clicked! canUndo:', canUndo.value);
      if (!canUndo.value) return;
      
      console.log('⬅️ Undo - Going to history index:', historyIndex.value - 1);
      historyIndex.value--;
    ...
    // Redo function
    const redo = () => {
      console.log('➡️ Redo clicked! canRedo:', canRedo.value);
      if (!canRedo.value) return;
      
      console.log('➡️ Redo - Going to history index:', historyIndex.value + 1);
      historyIndex.value++;
      const state = history.value[historyIndex.value];
      isRestoring.value = true;
      restoreState(state);
    ...
    const restoreState = (state) => {
      ...
      // Restore receipt-specific fields from the per-page data
      if (state.date !== undefined) date.value = state.date;
      if (state.receivedFrom !== undefined) receivedFrom.value = state.receivedFrom;
      if (state.paymentFor !== undefined) paymentFor.value = state.paymentFor;
      if (state.paymentFor2 !== undefined) paymentFor2.value = state.paymentFor2;
      if (state.naira !== undefined) naira.value = state.naira;
      
      // If we have complete page data, restore it directly
      if (state.currentPageData) {
        pageData.value[currentPage.value] = JSON.parse(JSON.stringify(state.currentPageData));
      }
  ...
  const showPreview = ref(true);
    ...
    const loadSharedSignatureState = () => {
      try {
        ...
        if (sharedData) {
          ...
          console.log('📋 Loaded shared signature state for receipt preview:', parsed);
        }
      } catch (error) {
    ...
    const saveSharedSignatureState = () => {
      try {
        ...
        console.log('💾 Saved shared signature state for receipt preview:', sharedData);
      } catch (error) {
    ...
    const logoHeight = computed(() => {
      const height = invoiceHeight.value;
      if (height >= 8) {
        return 70; // Full size for 8+ inches
      } else {
        // Reduce very slowly - only 1px per inch below 8
        const scaledHeight = 80 - ((8 - height) * 1);
    ...
    // Watch for changes to save state for undo/redo (debounced to avoid too many history entries)
    let saveStateTimeout = null;
    const debouncedSaveState = () => {
      console.log('🔄 debouncedSaveState called, isRestoring:', isRestoring.value);
      // Don't save state during restoration
      if (isRestoring.value) return;
      
      if (saveStateTimeout) clearTimeout(saveStateTimeout);
      saveStateTimeout = setTimeout(() => {
        console.log('⏰ Debounced timeout fired, calling saveState');
        saveState();
    ...
    // Also watch the underlying pageData for deep changes
    watch(pageData, () => {
      debouncedSaveState();
    }, { deep: true });
    ...
    // Stable font size for organization name with smooth scaling
    const organizationNameFontSize = computed(() => {
      return '16px'; // Fixed font size for consistency across all devices
    });
    ...
    const addressFontSize = computed(() => {
      const height = invoiceHeight.value;
      if (height >= 8) {
        return '14px'; // Increased from 11px for better visibility
      } else {
        // Reduce by 0.4px per inch below 8
        const scaledSize = 14 - ((8 - height) * 0.4);
        return `${Math.max(12, scaledSize)}px`; // Increased minimum from 10px to 12px
      }
    });
      ...
      try {
        ...
        if (savedDataString && savedDataString.trim() !== '') {
          ...
          try {
            ...
            // Load selectedTemplate if provided
            if (parsed.selectedTemplate) {
              selectedTemplate.value = parsed.selectedTemplate;
              console.log('🎨 Loaded selectedTemplate:', parsed.selectedTemplate);
            }
            ...
            if (quickSettingsString) {
              try {
                ...
                // Load element styles but exclude receipt input fields
                if (fontSettings.elementStyles) {
                  const excludedRefs = ['receivedFromInput', 'sumOfInput1', 'sumOfInput2', 'paymentForInput1', 'paymentForInput2'];
                  const filteredStyles = {};
                  
                  Object.keys(fontSettings.elementStyles).forEach(key => {
                    // Skip if this is a receipt input field
                    if (!excludedRefs.includes(key)) {
                      filteredStyles[key] = fontSettings.elementStyles[key];
                    }
                  });
                  
                  elementStyles.value = filteredStyles;
                }
                
                if (fontSettings.autoAdjustedElements) {
                  const excludedRefs = ['receivedFromInput', 'sumOfInput1', 'sumOfInput2', 'paymentForInput1', 'paymentForInput2'];
                  const filteredAutoAdjusted = fontSettings.autoAdjustedElements.filter(id => !excludedRefs.includes(id));
                  autoAdjustedElements.value = new Set(filteredAutoAdjusted);
                }
                
                if (fontSettings.originalElementStyles) {
                  const excludedRefs = ['receivedFromInput', 'sumOfInput1', 'sumOfInput2', 'paymentForInput1', 'paymentForInput2'];
                  const filteredOriginalStyles = {};
                  
                  Object.keys(fontSettings.originalElementStyles).forEach(key => {
                    // Skip if this is a receipt input field
                    if (!excludedRefs.includes(key)) {
                      filteredOriginalStyles[key] = fontSettings.originalElementStyles[key];
                    }
                  });
                  
                  originalElementStyles.value = filteredOriginalStyles;
                }
              } catch (fontError) {
            ...
            // Load receipt-specific fields
            if (parsed.receivedFrom !== undefined) receivedFrom.value = parsed.receivedFrom;
            if (parsed.paymentFor !== undefined) paymentFor.value = parsed.paymentFor;
            if (parsed.paymentFor2 !== undefined) paymentFor2.value = parsed.paymentFor2;
            if (parsed.naira !== undefined) naira.value = parsed.naira;
        ...
        // Add window focus event listener to reload signatures when returning to tab
        ...
        // Protect receipt input styles from any modifications
        setTimeout(() => {
          protectReceiptInputStyles();
          console.log('🛡️ Receipt input styles protected');
        }, 100);
    ...
    // Watch for organization name changes - simplified
    watch(
      () => organizationName.value,
      () => {
        nextTick(() => {
          const el = invoiceRef.value?.querySelector('.organization-name');
          if (!el) return;
          
          // Only set what must be dynamic
          el.style.fontSize = organizationNameFontSize.value;
          el.style.lineHeight = '1.25';
          
          console.log(`📏 Org name size: ${organizationNameFontSize.value}`);
        });
      },
      { flush: 'post' }
    );
    ...
    // Watch route changes to reload signatures when returning from signature page
    watch(() => route.fullPath, (newPath, oldPath) => {
      // Check if we're returning from signature page and reload signatures
      if (oldPath && oldPath.includes('/signature') && newPath && !newPath.includes('/signature')) {
    ...
    // Update amount in words based on naira value
    const updateAmountInWords = () => {
      if (!naira.value || naira.value <= 0) {
        sumOf.value = '';
        sumOf2.value = '';
        return;
      }
      
      const amountText = amountInWords.value + ' Only';
      
      // Check if the text fits in the first field (approximate character limit)
      const maxFirstFieldLength = 60; // Adjust based on your field width
      
      if (amountText.length <= maxFirstFieldLength) {
        sumOf.value = amountText;
        sumOf2.value = '';
      } else {
        // Split the text intelligently at word boundaries
        const words = amountText.split(' ');
        let firstPart = '';
        let secondPart = '';
        
        for (let i = 0; i < words.length; i++) {
          const testFirstPart = firstPart + (firstPart ? ' ' : '') + words[i];
          if (testFirstPart.length <= maxFirstFieldLength) {
            firstPart = testFirstPart;
          } else {
        ...
        sumOf.value = firstPart;
        sumOf2.value = secondPart;
      }
    };
    
    // Watch naira amount changes to auto-update amount in words
    watch(naira, (newAmount) => {
      updateAmountInWords();
    }, { immediate: true });
    ...
    // Auto-update amount in words based on naira (for receipts) or grand total (for invoices)
    const amountInWords = computed(() => {
      // For receipts, use the naira field directly (only if it has a meaningful value)
      if (naira.value && naira.value > 0) {
        return numberToWords(naira.value);
      }
      // For invoices, use grand total from items (only if it has a meaningful value)
      ...
      if (tempNum === 0) {
        ...
        while (tempNum > 0) {
          ...
          if (chunk !== 0) {
            ...
            if (chunkWords && typeof chunkWords === 'string') {
              if (scales[scaleIndex]) {
                parts.unshift((chunkWords + ' ' + scales[scaleIndex]).trim());
              } else {
        ...
        result = parts.filter(part => part && typeof part === 'string').join(' ');
      }
    ...
    // Protect receipt input styles from any modifications
    const protectReceiptInputStyles = () => {
      const receiptInputRefs = [
        receivedFromInput,
        sumOfInput1,
        sumOfInput2,
        paymentForInput1,
      ...
      receiptInputRefs.forEach(refObj => {
        if (refObj?.value) {
          const element = refObj.value;
          // Force apply correct styles
          Object.assign(element.style, correctStyle);
          
          // Watch for any style changes and revert them
          const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
              if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                ...
                if (currentStyle.fontSize !== '16px') needsCorrection = true;
                if (currentStyle.fontWeight !== '700') needsCorrection = true;
                if (currentStyle.fontFamily !== 'Georgia, serif') needsCorrection = true;
                if (currentStyle.fontStyle !== 'italic') needsCorrection = true;
                
                if (needsCorrection) {
                  console.warn('⚠️ Receipt input style was modified, reverting to correct style');
                  Object.assign(element.style, correctStyle);
                }
              }
            });
    ...
    const saveQuickSettings = () => {
      ...
      saveQuickSettingsTimeout = setTimeout(() => {
        // Filter out receipt input fields from elementStyles before saving
        const excludedRefs = ['receivedFromInput', 'sumOfInput1', 'sumOfInput2', 'paymentForInput1', 'paymentForInput2'];
        const filteredElementStyles = {};
        
        Object.keys(elementStyles.value).forEach(key => {
          if (!excludedRefs.includes(key)) {
            filteredElementStyles[key] = elementStyles.value[key];
          }
        });
        
        // Filter out receipt input fields from autoAdjustedElements
        const filteredAutoAdjusted = Array.from(autoAdjustedElements.value).filter(id => !excludedRefs.includes(id));
        
        // Filter out receipt input fields from originalElementStyles
        const filteredOriginalStyles = {};
        Object.keys(originalElementStyles.value).forEach(key => {
          if (!excludedRefs.includes(key)) {
            filteredOriginalStyles[key] = originalElementStyles.value[key];
          }
        });
        ...
        const quickSettings = {
          ...
          customColor1CMYK: customColor1CMYK.value,
          customColor2CMYK: customColor2CMYK.value,
          elementStyles: filteredElementStyles,
          autoAdjustedElements: filteredAutoAdjusted,
          originalElementStyles: filteredOriginalStyles
        };
        safeLocalStorage.setItem('invoiceQuickSettings', JSON.stringify(quickSettings));
        console.log('✅ Saved invoice settings to localStorage (receipt inputs excluded)');
        saveQuickSettingsTimeout = null;
      }, 500);
    };
    ...
    const setupAutoAdjustment1 = () => {
      ...
      // Exclude receipt input fields from auto-adjustment (they have their own fixed styling)
      const excludedRefs = ['receivedFromInput', 'sumOfInput1', 'sumOfInput2', 'paymentForInput1', 'paymentForInput2'];
      
      textElements.forEach((element, index) => {
        // Skip receipt input fields that should maintain their fixed styling
        const refAttr = element.getAttribute('ref');
        if (refAttr && excludedRefs.includes(refAttr)) {
      ...
      try {
        ...
        if (format === 'pdf') {
          ...
          if (format === 'jpeg') {
            ...
            fileName = customName ? `${customName}.jpg` : `Receipt-${Date.now()}.jpg`;
          } else {
            originalBuffer = await new Promise(resolve => {
    ...
    const handleExportPDF = async (exportType = 'current', customName = null) => {
      ...
      try {
        ...
        if (exportType === 'all') {
          ...
          try {
            ...
            for (let page = 1; page <= totalCopies.value; page++) {
              ...
              // Capture current page using htmlToImage for better quality
            ...
            // Check if running on native platform (APK)
            const isNative = Capacitor.isNativePlatform();
            
            if (isNative) {
              // For Android/iOS: Save PDF to External storage then share
              try {
                ...
                await Share.share({
                  title: 'Receipt PDF',
                  text: `${filename} (${pagesToExport} pages)`,
                  url: savedFile.uri,
              ...
              try {
                ...
                // Alternative method for web browsers
        ...
        // Capture using htmlToImage for better quality
        const imgData = await htmlToImage.toJpeg(targetElement, {
          quality: 0.98,
          pixelRatio: 3,
          cacheBust: true,
        ...
        // Log export activity
        const member = JSON.parse(safeLocalStorage.getItem('sdp_authenticated_member') || '{}');
    ...
    const handleExportJPEG = async (exportType = 'current', customName = null) => {
      ...
      let originalPadding = '';
      let contentWrapperOriginalTransform = '';
      ...
      try {
        ...
        originalMargin = invoiceRef.value.style.margin;
        originalWidth = invoiceRef.value.style.width;
        originalBoxShadow = invoiceRef.value.style.boxShadow;
        originalPadding = invoiceRef.value.style.padding;
        
        // Store contentWrapperRef transform
        if (contentWrapperRef.value) {
          contentWrapperOriginalTransform = contentWrapperRef.value.style.transform;
        }
        ...
        // Apply clean export styles for JPEG - optimize padding for perfect centering
        invoiceRef.value.style.boxShadow = 'none';
        invoiceRef.value.style.padding = '0.2in';
        invoiceRef.value.style.overflow = 'hidden';
        invoiceRef.value.style.height = `${invoiceHeight.value}in`;
        
        // Reset contentWrapperRef transform to prevent scaling
        if (contentWrapperRef.value) {
          contentWrapperRef.value.style.transform = 'none';
        }
        
        // Adjust inner content wrapper for better centering during export
        const contentWrapper = invoiceRef.value.querySelector('.invoice-content-wrapper');
        let originalContentPadding = {};
        if (contentWrapper) {
          // Store original padding
          originalContentPadding = {
            paddingTop: contentWrapper.style.paddingTop,
            paddingBottom: contentWrapper.style.paddingBottom,
            paddingLeft: contentWrapper.style.paddingLeft,
            paddingRight: contentWrapper.style.paddingRight
          };
          // Apply balanced padding for export
          contentWrapper.style.paddingTop = '0.08in';
          contentWrapper.style.paddingBottom = '0.08in';
          contentWrapper.style.paddingLeft = '0.15in';
          contentWrapper.style.paddingRight = '0.15in';
        }
        
        // Force a reflow to ensure styles are applied
        void invoiceRef.value.offsetHeight;
        ...
        if (exportType === 'all' && totalCopies.value > 1) {
          ...
          for (let page = 1; page <= totalCopies.value; page++) {
            ...
            const dataUrl = await htmlToImage.toJpeg(invoiceRef.value, { 
              quality: 0.98, 
              pixelRatio: 3,
              width: invoiceWidth.value * 96,
              height: invoiceHeight.value * 96,
              cacheBust: true,
              backgroundColor: '#FFFFFF',
              skipFonts: true,
              fontEmbedCSS: '',
            ...
            // Store download info
            const baseFilename = customName || `Receipt-${(receiptNumber.value || 1) + page - 1}`;
            ...
            try {
              ...
              for (const download of downloads) {
                ...
                try {
                  ...
                  // Fallback: Save to External storage for sharing
                  const savedFile = await Filesystem.writeFile({
                    path: download.filename,
                    data: base64Data,
                    directory: Directory.External,
          ...
          const filename = customName ? `${customName}.jpg` : `Receipt-${currentInvoiceNumber.value.replace('/', '-')}.jpg`;
          ...
          if (isNative) {
            ...
            try {
              ...
              try {
                ...
                // Fallback: Save to External storage then share
                const savedFile = await Filesystem.writeFile({
                  path: filename,
                  data: base64Data,
                  directory: Directory.External,
                ...
                await Share.share({
                  ...
                  text: filename,
                  url: savedFile.uri,
                  dialogTitle: 'Share Receipt JPEG'
                });
                
                console.log('✅ JPEG shared successfully!');
              }
        ...
        if (invoiceRef.value) {
          ...
          invoiceRef.value.style.margin = originalMargin;
          invoiceRef.value.style.width = originalWidth;
          invoiceRef.value.style.boxShadow = originalBoxShadow;
          invoiceRef.value.style.padding = originalPadding;
          
          // Restore contentWrapperRef transform
          if (contentWrapperRef.value) {
            contentWrapperRef.value.style.transform = contentWrapperOriginalTransform;
          }
          
          // Restore inner content wrapper padding
          const contentWrapper = invoiceRef.value.querySelector('.invoice-content-wrapper');
          if (contentWrapper && originalContentPadding) {
            contentWrapper.style.paddingTop = originalContentPadding.paddingTop || '';
            contentWrapper.style.paddingBottom = originalContentPadding.paddingBottom || '';
            contentWrapper.style.paddingLeft = originalContentPadding.paddingLeft || '';
            contentWrapper.style.paddingRight = originalContentPadding.paddingRight || '';
          }
    ...
    const handleSaveInvoice = async () => {
      ...
      try {
        ...
        const memberDataString = safeLocalStorage.getItem('sdp_authenticated_member');
    ...
    const handleSaveReceipt = async () => {
      ...
      try {
        ...
        // Prepare receipt data to save (receipt-specific fields)
        const receiptToSave = {
          organizationName: organizationName.value,
          organizationSubName: organizationSubName.value,
          headOfficeAddress: headOfficeAddress.value,
        ...
        // Save to localStorage (current receipt being edited)
        safeLocalStorage.setItem('receiptPreviewData', JSON.stringify(receiptToSave));
        ...
        if (result.success) {
          const viewSaved = confirm(`✅ Receipt saved successfully!\n\nReceipt ID: ${result.id}\nBranch: ${member.branch}\n\nYour receipt is saved in:\n- localStorage (key: "sdp_receipts_${member.branch}")\n- Browser Storage under your branch\n\nWould you like to view all saved receipts in console?`);
          
          if (viewSaved) {
            // Get all saved receipts
            const { getAllReceipts } = await import('@/firebase/database');
            const allReceipts = await getAllReceipts(member.branch);
            /* eslint-disable no-console */
            console.log('=== SAVED RECEIPTS ===');
            console.log(`Branch: ${member.branch}`);
            console.log(`Total Receipts: ${allReceipts.receipts?.length || 0}`);
            console.table(allReceipts.receipts?.map(receipt => ({
              ID: receipt.id,
              Receipt_Number: receipt.receiptNumber,
              Received_From: receipt.receivedFrom,
              Payment_For: receipt.paymentFor,
              Amount: receipt.naira,
              Total: receipt.grandTotal,
              Date: receipt.receiptDate,
              Created: new Date(receipt.createdAt).toLocaleString()
            })));
            /* eslint-enable no-console */
            alert('Check your browser console (F12) to see all saved receipts!');
          }
        } else {
          alert(`⚠️ Receipt saved locally but Firebase save failed: ${result.error}\n\nYou can still access it from localStorage key: "sdp_receipts_${member.branch}"`);
        }
      } catch (error) {
    ...
    const handleSumOfOverflow = () => {
      // Auto-populate from amount in words
      updateAmountInWords();
    };
    ...
    // Handle payment for overflow
    const handlePaymentForOverflow = () => {
      if (!paymentForInput1.value) return;
      
      const input = paymentForInput1.value;
      const maxWidth = input.scrollWidth;
      const currentWidth = input.clientWidth;
      
      // Check if text is overflowing (scrollWidth > clientWidth means overflow)
      if (input.scrollWidth > input.clientWidth) {
        // Split text at last space before overflow
        const text = paymentFor.value;
        const words = text.split(' ');
        
        // Find optimal split point by measuring text width
        ...
        input.value = paymentFor.value;
        
        if (splitIndex > 0 && splitIndex < words.length) {
          // Move overflow text to second input
          paymentFor.value = words.slice(0, splitIndex).join(' ');
          paymentFor2.value = words.slice(splitIndex).join(' ');
          ...
          nextTick(() => {
            if (paymentForInput2.value) {
              ...
              // Set cursor to end
              const len = paymentFor2.value.length;
              paymentForInput2.value.setSelectionRange(len, len);
            }
          });
        }
      }
    ...
    // Handle payment for 2 input
    const handlePaymentFor2Input = () => {
      // If second input is empty and user presses backspace, move focus back to first input
      if (paymentFor2.value === '' && paymentForInput1.value) {
        paymentForInput1.value.focus();
      }
    };
    ...
    const loadSignatures = async () => {
      try {
        console.log('🖋️ [PreviewReceipt] Loading signatures...');
        
        const memberData = await withFirebaseErrorHandling(
          async () => safeLocalStorage.getItem('sdp_authenticated_member'),
          'loadMemberData',
          { suppressConsoleErrors: true }
        );
        
        if (!memberData) {
          console.log('🖋️ [PreviewReceipt] No authenticated member data available');
          return;
        }
        ...
        try {
          ...
          console.log('🖋️ [PreviewReceipt] Error parsing member data');
          return;
        }
        
        if (!member?.branch) {
          console.log('🖋️ [PreviewReceipt] No branch found in member data');
          return;
        }

        console.log('🖋️ [PreviewReceipt] Loading signatures for branch:', member.branch);

        // Use dynamic import like ReceiptPage.vue
        const { getAllSignatures } = await import('@/firebase/database');
        const result = await getAllSignatures(member.branch);
        
        console.log('🔥 [PreviewReceipt] Firebase getAllSignatures result:', result);
        
        if (result?.success) {
          savedSignatures.value = result.signatures || result.data || [];
          console.log('✅ [PreviewReceipt] Loaded signatures count:', savedSignatures.value.length);
          console.log('📝 [PreviewReceipt] Signature details:', savedSignatures.value.map(s => ({ 
            id: s.id, 
            name: s.name,
          ...
          if (primary) {
            console.log('🖋️ [PreviewReceipt] Setting primary signature:', primary.name);
            ...
            console.log('🖋️ [PreviewReceipt] No primary signature, using first available');
          ...
          if (savedSignatures.value.length === 0) {
            console.log('🗃️ [PreviewReceipt] No Firebase signatures, checking localStorage...');
            const signatureKeys = Object.keys(localStorage).filter(key => 
              key.includes('signature') || key.includes('Signature')
            );
            console.log('🗃️ [PreviewReceipt] Found localStorage signature keys:', signatureKeys);
              ...
              try {
                ...
                signatureKeys.forEach(key => {
                  try {
                    ...
                    console.log('🗃️ [PreviewReceipt] Failed to parse localStorage signature:', key);
                  }
                });
                
                if (localSignatures.length > 0) {
                  console.log('📦 [PreviewReceipt] Using localStorage signatures:', localSignatures.length);
                  savedSignatures.value = localSignatures;
                }
              } catch (error) {
                console.error('🗃️ [PreviewReceipt] Error processing localStorage signatures:', error);
              }
            }
          }
        } else {
          console.log('❌ [PreviewReceipt] Failed to load signatures from Firebase');
          savedSignatures.value = [];
        }
      } catch (error) {
        console.error('❌ [PreviewReceipt] Error in loadSignatures:', error);
        savedSignatures.value = [];
      }
    };
    ...
    const viewSavedInvoices = () => {
      ...
      router.push({ path: '/saved-invoices', query: { branch } });
    };

    const viewSavedReceipts = () => {
      const branch = router.currentRoute.value.query.branch || '';
      router.push({ path: '/saved-receipts', query: { branch } });
    };
    
    // Save Receipt Functions
    const showSaveDialog = () => {
      // Generate a default receipt name based on customer and date
      const from = receivedFrom.value || organizationName.value || 'Receipt';
    ...
    const saveReceipt = async () => {
      ...
      try {
        ...
        const memberData = localStorage.getItem('authenticatedMember') || localStorage.getItem('sdp_authenticated_member');
        if (!memberData) {
          alert('Please log in to save receipts');
          return;
        }
        ...
        if (!member.branch) {
          alert('No branch information found. Please log in again.');
          return;
        }
        ...
        const receiptData = {
          ...
          paymentFor: paymentFor.value,
          date: date.value,
          receiptNumber: currentInvoiceNumber.value,
          naira: naira.value,
          kobo: 0, // Kobo field not used in this receipt template
          branch: member.branch,
        ...
        if (result.success) {
          // Log activity
          try {
            const { logActivity } = await import('../../api-service');
            await logActivity(member.branch, {
              action: `Receipt Saved: ${receiptName.value}`,
              memberName: member.name || 'Unknown Member',
              type: 'receipt',
              details: {
                receiptNumber: currentInvoiceNumber.value,
          ...
          // Navigate to saved receipts page
          const branch = router.currentRoute.value.query.branch || '';
          router.push({ path: '/saved-receipts', query: { branch } });
        } else {
    ...
    const createNewInvoice = () => {
      if (confirm('Are you sure you want to start a new invoice? Any unsaved changes will be lost.')) {
        ...
        router.push({ name: 'Invoice', query: router.currentRoute.value.query });
      }
    };

    const createNewReceipt = () => {
      if (confirm('Are you sure you want to start a new receipt? Any unsaved changes will be lost.')) {
        ...
        // Reset receipt number to auto-generate
        if (autoReceiptNumber.value) {
          const today = new Date();
          const timestamp = today.getTime();
          const randomSuffix = Math.floor(Math.random() * 1000).toString().padStart(3, '0');
          receiptNumber.value = `RCP-${today.getFullYear()}-${timestamp}-${randomSuffix}`;
        }
        
        // Reset date if auto-date is enabled
        if (autoDate.value) {
          const today = new Date();
          date.value = today.toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit', 
            year: 'numeric'
          });
        ...
        // Navigate to new receipt creation
        router.push({ name: 'Receipt', query: router.currentRoute.value.query });
      }
    };
    ...
    const zoomIn = () => {
      const increment = 0.1;
      const newZoom = Math.min(maxZoom.value, zoomLevel.value + increment);
    ...
    const zoomOut = () => {
      const decrement = 0.1;
      const newZoom = Math.max(minZoom.value, zoomLevel.value - decrement);
      if (newZoom !== zoomLevel.value) {
        zoomLevel.value = newZoom;
        triggerBounce();
      }
    ...
    const autoFitZoom = () => {
      if (!previewContainerRef.value || !invoiceRef.value) return;
      ...
      // Convert invoice dimensions from inches to pixels (assuming 96 DPI)
      const invoicePixelWidth = invoiceWidth.value * 96;
      const invoicePixelHeight = invoiceHeight.value * 96;
      
      // Calculate scale factors for both dimensions
      const scaleX = containerWidth / invoicePixelWidth;
      const scaleY = containerHeight / invoicePixelHeight;
      
      // Set to 50% zoom for auto-fit
      const fitZoom = 0.5;
      
      // Constrain to min/max zoom levels
      zoomLevel.value = Math.max(minZoom.value, Math.min(maxZoom.value, fitZoom));
      triggerBounce();
    };
    ...
    const handleSliderTouchStart = (e) => {
      e.preventDefault();
      isSliderDragging = true;
      updateZoomFromSlider(e.touches[0]);
    };
    ...
    const updateZoomFromSlider = (event) => {
      ...
      // Invert the calculation since slider goes from top (max) to bottom (min)
      const percentage = Math.max(0, Math.min(100, ((height - y) / height) * 100));
      
      const newZoom = minZoom.value + (percentage / 100) * (maxZoom.value - minZoom.value);
      zoomLevel.value = Math.max(minZoom.value, Math.min(maxZoom.value, newZoom));
      triggerBounce();
    };
...
/* Using universally available system fonts for maximum compatibility */
.organization-name {
...
/* Advanced zoom area optimization for APK/mobile */
.zoom-area {
...
/* Bounce animation for zoom slider */
.scale-y-110 {
