<template>
    ...
    <div class="flex justify-center items-center my-1">
      <div class="flex items-center justify-center gap-4">
        ...
        <div class="flex items-center gap-1">
          <span class="text-base font-bold text-black">No.:</span>
  ...
  <div class="my-0 grid gap-1" :class="invoiceWidth >= 4.5 ? 'grid-cols-3' : 'grid-cols-1'" :style="{ paddingLeft: '0px', paddingRight: '0px' }">
    <div 
      ...
      class="rounded-xl"
      :style="{ border: '1.5px solid #000000', padding: invoiceWidth >= 4 ? '4px' : '3px' }"
    >
  ...
  <div class="flex-grow overflow-visible rounded relative isolate" :style="{ marginTop: dynamicSpacing, minHeight: 'fit-content', paddingLeft: '0px', paddingRight: '0px', paddingBottom: '0px' }">
    <table class="w-full text-xs table-fixed border-collapse overflow-visible relative" style="table-layout: fixed; position: relative;">
      ...
      <tbody style="position: relative; z-index: 1;">
        <tr v-for="(item, index) in items" :key="item.id" class="border-t border-black group relative" :style="{ height: tableRowHeight }">
          <td class="px-1.5 py-0.1 text-center align-middle border-black" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto text-center bg-transparent focus:outline-none focus:ring-2 focus:ring-gray-400 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td class="px-1.5 py-0.1 align-middle border-black" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto bg-transparent focus:outline-none focus:ring-2 focus:ring-gray-400 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td class="px-1.5 py-0.1 text-right align-middle border-black" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto text-right bg-transparent focus:outline-none focus:ring-2 focus:ring-gray-400 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td v-if="invoiceData.taxEnabled" class="px-1.5 py-0.1 text-center align-middle border-black" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto text-center bg-transparent focus:outline-none focus:ring-2 focus:ring-gray-400 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td class="px-0 py-0.1 align-middle border-black relative" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div class="flex h-full">
              <!-- Naira Cell (Whole Number) - Light grey background for B&W -->
              <div class="flex-1 px-1.5 py-0.1 border-r border-black flex items-center justify-center" style="background-color: #E8E8E8;">
                <span class="text-[11px] text-black">{{ getItemAmount(item) > 0 ? Math.floor(getItemAmount(item)) : '' }}</span>
              </div>
              <!-- Kobo Cell (Decimal Part) - Light grey background for B&W -->
              <div :style="{ width: koboColumnWidth }" class="px-1.5 py-0.1 flex items-center justify-center" style="background-color: #E8E8E8;">
            ...
            <button 
              v-if="items.length > 1"
              class="absolute right-[-8px] top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-800 opacity-0 group-hover:opacity-100 transition-all duration-200 w-4 h-4 flex items-center justify-center text-sm font-bold hover:scale-110 bg-white rounded-full shadow-md border border-gray-400"
              style="z-index: 9999;"
              title="Remove item"
    ...
    <div class="flex-1 justify-end flex">
      <span class="mr-2 flex items-center px-1.5 py-1 font-black align-middle text-[13px] text-black" data-text-id="total-label" :style="{ height: tableRowHeight }">TOTAL:</span>
      ...
      <div class="px-0 py-0.1 font-semibold align-middle relative overflow-visible text-[11px]" :style="{ height: tableRowHeight, border: '1px solid #000000', width: totalBoxWidth, minWidth: totalBoxMinWidth }">
        <div class="flex h-full">
          <!-- Naira Total - Grey background for B&W -->
          <div class="flex-1 px-1.5 py-0.1 flex items-center justify-center border-r border-black" style="background-color: #E8E8E8;">
            <span class="font-bold text-sm text-black">{{ totalNaira > 0 && !isNaN(totalNaira) ? '₦' + Number(totalNaira) : '' }}</span>
          </div>
          <!-- Kobo Total - Grey background for B&W -->
          <div :style="{ width: koboTotalWidth }" class="px-1.5 py-0.1 flex items-center justify-center" style="background-color: #E8E8E8;">
     ...
     <div class="flex justify-between items-start" :style="{ marginTop: '2px', minHeight: 0, flex: '0 0 auto' }">
      ...
      <div class="flex flex-col items-center gap-1 mt-[-15px]">
        ...
        <div v-if="signatureImage1" class="flex items-center" :style="{ height: signatureHeight }">
          <img :src="signatureImage1" alt="Signature 1" class="w-auto object-contain" :style="{ height: signatureImageHeight, maxWidth: '120px', filter: 'grayscale(100%)' }" />
        </div>
      ...
      <!-- Thanks for your patronage -->
       <div class="mt-2 text-center font-medium text-[13px]" style="color: #000000;">Thanks for your patronage</div>

    <!-- Signature 2 -->
      <div class="flex flex-col items-center gap-1 mt-[-15px]">
        <!-- Signature 2 Image -->
        <div v-if="signatureImage2" class="flex items-center" :style="{ height: signatureHeight }">
          <img :src="signatureImage2" alt="Signature 2" class="w-auto object-contain" :style="{ height: signatureImageHeight, maxWidth: '120px', filter: 'grayscale(100%)' }" />
        </div>
...
export default defineComponent({
  ...
  props: {
    ...
    // Colors - accepted but overridden to B&W internally
