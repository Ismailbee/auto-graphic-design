<template>
  ...
  <div class="text-center flex-shrink-0" :style="{ paddingBottom: '4px' }">
    ...
    <div class="flex gap-3 items-start">
      ...
      <div class="flex-shrink-0 flex items-center justify-center">
        <div v-if="!logoDataUrl" class="text-xs text-gray-500 border border-gray-400 p-1">
          No Logo
        </div>
        <img 
          v-if="logoDataUrl"
    ...
    <div v-if="headOfficeAddress?.trim() || headOfficePhone?.trim()" class="flex items-start justify-center mt-1 gap-6">
      ...
      <div 
        ...
        class="text-left mt-0 min-w-[110px] pl-3"
        :style="{ borderLeft: '1px solid #C80032', lineHeight: '1.1' }"
      >
    ...
    <div class="flex justify-center items-center my-1">
      <div class="flex items-center justify-center gap-4">
        ...
        <div class="flex items-center gap-1">
          <span class="text-base font-bold" :style="{ color: '#C80032' }">No.:</span>
          <div v-if="showPageNumbers" class="text-center text-lg font-bold text-black">
  ...
  <div class="my-0 grid gap-1" :class="invoiceWidth >= 4.5 ? 'grid-cols-3' : 'grid-cols-1'" :style="{ paddingLeft: '0px', paddingRight: '0px' }">
    <div 
      ...
      class="rounded-xl"
      :style="{ border: '1.5px solid #C80032', padding: invoiceWidth >= 4 ? '4px' : '3px' }"
    >
      <div class="flex items-center gap-0.5" :class="invoiceWidth < 4 ? 'flex-col items-start gap-0' : ''">
      <span class="font-medium whitespace-nowrap" :style="{ fontSize: customerDetailsFontSize, color: '#C80032' }">Name:</span>
    ...
    <div class="flex items-center gap-0.5 mt-0.5" :class="invoiceWidth < 4 ? 'flex-col items-start gap-0' : ''">
      <span class="font-medium whitespace-nowrap" :style="{ fontSize: customerDetailsFontSize, color: '#C80032' }">Address:</span>
    ...
    >
      <div class="flex items-center gap-0.5" :class="invoiceWidth < 4 ? 'flex-col items-start gap-0' : ''">
     <span class="font-medium whitespace-nowrap" :style="{ fontSize: customerDetailsFontSize, color: '#C80032' }">Date:</span>
     <div class="print-only w-full border-b border-dotted border-black" :style="{ borderBottom: '1px dotted #000000 !important', fontSize: customerDetailsFontSize }">
    ...
    <div class="flex items-center gap-0.5 mt-2" :class="invoiceWidth < 4 ? 'flex-col items-start gap-0' : ''">
      <span class="font-medium whitespace-nowrap" :style="{ fontSize: customerDetailsFontSize, color: '#C80032' }">L.P.O No.:</span>
      <div class="print-only w-full border-b border-dotted border-black" :style="{ borderBottom: '1px dotted #000000 !important', fontSize: customerDetailsFontSize }">{{ localLpo || '-' }}</div>
      <input
        v-model="localLpo"
        placeholder=" "
  ...
  <div class="flex-grow overflow-visible rounded relative isolate" :style="{ marginTop: dynamicSpacing, minHeight: 'fit-content', paddingLeft: '0px', paddingRight: '0px', paddingBottom: '0px' }">
    <table class="w-full text-xs table-fixed border-collapse overflow-visible relative" style="table-layout: fixed; position: relative;">
      ...
      >
        <tr>
          <th 
            ...
            :style="{ 
              borderTop: '1px solid #C80032', 
              borderRight: '1px solid #FFFFFF', 
              borderBottom: '1px solid #C80032', 
              borderLeft: '1px solid #C80032',
      ...
      <tbody style="position: relative; z-index: 1;">
        <tr v-for="(item, index) in items" :key="item.id" class="border-t border-black group relative" :style="{ height: tableRowHeight }">
          <td class="px-1.5 py-0.1 text-center align-middle" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto text-center bg-transparent focus:outline-none focus:ring-2 focus:ring-red-300 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td class="px-1.5 py-0.1 align-middle" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto bg-transparent focus:outline-none focus:ring-2 focus:ring-red-300 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td class="px-1.5 py-0.1 text-right align-middle" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto text-right bg-transparent focus:outline-none focus:ring-2 focus:ring-red-300 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td v-if="invoiceData.taxEnabled" class="px-1.5 py-0.1 text-center align-middle" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div 
              class="w-full h-auto text-center bg-transparent focus:outline-none focus:ring-2 focus:ring-red-300 rounded text-[11px] leading-tight px-1 py-0.5"
          ...
          <td class="px-0 py-0.1 align-middle relative" :style="{ height: tableRowHeight, border: '1px solid #000000' }">
            <div class="flex h-full">
              <!-- Naira Cell - Light red/pink tint -->
              <div class="flex-1 px-1.5 py-0.1 border-r border-black flex items-center justify-center" style="background-color: #FDE8ED;">
                <span class="text-[11px] text-black">{{ getItemAmount(item) > 0 ? Math.floor(getItemAmount(item)) : '' }}</span>
              </div>
              <!-- Kobo Cell - Light red/pink tint -->
              <div :style="{ width: koboColumnWidth }" class="px-1.5 py-0.1 flex items-center justify-center" style="background-color: #FDE8ED;">
                <span class="text-[11px] text-black">{{ getItemAmount(item) > 0 ? Math.round((getItemAmount(item) % 1) * 100) : '' }}</span>
              </div>
            </div>
            <!-- Delete button -->
            <button 
              v-if="items.length > 1"
              class="absolute right-[-8px] top-1/2 -translate-y-1/2 text-gray-500 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-all duration-200 w-4 h-4 flex items-center justify-center text-sm font-bold hover:scale-110 bg-white rounded-full shadow-md border border-gray-400"
              style="z-index: 9999;"
              title="Remove item"
    ...
    <div class="flex-1 justify-end flex">
      <span class="mr-2 flex items-center px-1.5 py-1 font-black align-middle text-[13px]" data-text-id="total-label" :style="{ height: tableRowHeight, color: '#C80032' }">TOTAL:</span>
      <!-- Combined Total Box -->
      <div class="px-0 py-0.1 font-semibold align-middle relative overflow-visible text-[11px]" :style="{ height: tableRowHeight, border: '1px solid #000000', width: totalBoxWidth, minWidth: totalBoxMinWidth }">
        <div class="flex h-full">
          <!-- Naira Total - Light red/pink tint -->
          <div class="flex-1 px-1.5 py-0.1 flex items-center justify-center border-r border-black" style="background-color: #FDE8ED;">
            <span class="font-bold text-sm text-black">{{ totalNaira > 0 && !isNaN(totalNaira) ? '₦' + Number(totalNaira) : '' }}</span>
          </div>
          <!-- Kobo Total - Light red/pink tint -->
          <div :style="{ width: koboTotalWidth }" class="px-1.5 py-0.1 flex items-center justify-center" style="background-color: #FDE8ED;">
            <span class="font-bold text-sm text-black">{{ totalKobo > 0 && !isNaN(totalKobo) ? Number(totalKobo) : '' }}</span>
          </div>
        </div>
      </div>
    </div>  
  ...
  <div class="mt-auto" :style="{ fontSize: footerFontSize }">
    ...
    <div>
      <div class="flex items-center gap-1">
      <span class="flex whitespace-nowrap font-medium text-black" data-text-id="amount-in-words-label">Amount in words:</span>
      <div class="print-only flex-1 border-b border-dotted border-black" :style="{ borderBottom: '1px dotted #000000 !important', fontSize: '1.2em', fontFamily: 'Montserrat, sans-serif', fontWeight: '700', wordWrap: 'break-word', whiteSpace: 'pre-wrap' }">{{ localSumOf ? localSumOf + ' Only' : '' }}</div>
      <input
        ref="sumOfInput1"
        v-model="localSumOf"
        class="no-print flex-1 bg-transparent border-b border-dotted border-black focus:outline-none"
    ...
    <div class="flex items-center h-7 gap-2">
      <div class="print-only flex-1 border-b border-dotted border-black" :style="{ borderBottom: '1px dotted #000000 !important', fontSize: '1.2em', fontFamily: 'Montserrat, sans-serif', fontWeight: '700', wordWrap: 'break-word', whiteSpace: 'pre-wrap' }">{{ localSumOf2 ? localSumOf2 + ' Only' : '' }}</div>
      <input
        ref="sumOfInput2"
        v-model="localSumOf2"
        type="text"
      ...
      />
      <span class="whitespace-nowrap font-medium inline-flex items-baseline text-black" :style="{ fontSize: footerFontSize, lineHeight: 1 }">Naira</span>
      ...
      <span class="whitespace-nowrap font-medium inline-flex items-baseline text-black" :style="{ fontSize: footerFontSize, lineHeight: 1 }">Kobo</span>
    </div>
     ...
     <div class="flex justify-between items-start" :style="{ marginTop: '2px', minHeight: 0, flex: '0 0 auto' }">
      ...
      <div class="flex flex-col items-center gap-1 mt-[-15px]">
       ...
       <div class="w-full text-center mt-[-28px]" :style="{ borderTop: '1px solid #000000' }">
         <p class="italic text-[10px] text-black">Manager's Sign</p>
       </div> 
      </div>

      <!-- Thanks for your patronage -->
       <div class="mt-2 text-center font-medium text-[13px]" :style="{ color: '#C80032' }">Thanks for your patronage</div>
    ...
    <!-- Signature 2 -->
      <div class="flex flex-col items-center gap-1 mt-[-15px]">
       ...
       <div class="w-full text-center mt-[-28px]" :style="{ borderTop: '1px solid #000000' }">
         <p class="italic text-[10px] text-black">Customer's Sign</p>
       </div> 
      </div>
...
export default defineComponent({
  ...
  props: {
    ...
    // Colors - accepted but uses 2-color scheme internally
