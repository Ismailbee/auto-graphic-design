/**
 * Tag Style 6 Generator (Blue Gradient)
 * REBUILT FROM SCRATCH - Following tag1Generator pattern exactly
 * Based on tag6.svg template with cyan/blue gradient design
 * 
 * ViewBox: 144188.47 x 203923.56 (71x larger than tag1's 2030.75 x 2872.06)
...
export interface Tag6Config {
  templateName: 'tag6.svg'
  displayName: 'Style 6 (Blue Gradient)'
  
  // Colors from tag6.svg
  primaryColor: '#FFFFFF'       // White for text on colored sections
  eventTypeColor: '#003D82'     // Dark blue for event type
  detailsColor: '#000000'       // Black for details on white section
  
  // Layout - from tag6.svg viewBox
  viewBox: '0 0 144188.47 203923.56'
  width: 144188.47
  height: 203923.56
  centerX: 72094.235
...
export const TAG6_CONFIG: Tag6Config = {
  templateName: 'tag6.svg',
  displayName: 'Style 6 (Blue Gradient)',
  primaryColor: '#FFFFFF',
  eventTypeColor: '#003D82',
  detailsColor: '#000000',
  viewBox: '0 0 144188.47 203923.56',
  width: 144188.47,
  height: 203923.56,
...
/**
 * Generate Tag Style 6
 * Clean implementation following tag1Generator pattern
 */
export async function generateTag6(
  svg: SVGSVGElement,
  ...
  console.log('=== TAG6 GENERATOR v6.0 - REBUILT FROM SCRATCH ===')
  
  const config = TAG6_CONFIG
  ...
  if (eventText) {
    // Position dynamically: at least in the white section area
    const eventStartY = Math.max(lastY + (50 * SCALE_FACTOR), 130000)
    ...
    console.log('Tag6 Event: text="' + eventText + '", chars=' + eventCharCount + ', fontSize=' + eventFontSize)
  ...
  const detailsStartY = Math.max(lastY, 140000)
  ...
  await nextTick()
  console.log('=== TAG6 GENERATION COMPLETE ===')
}
